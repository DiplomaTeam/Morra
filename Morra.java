...
// DECLARE AND INITIALISE VARIABLES
	// variables to store the user's input and computer imput
	...
	//variables for game history - arrays of size hundred to allow 25 games to be played
		...
		int[] historyOfChoicesComputer; historyOfChoicesComputer = new int [100]; ////history of what choises computer made
		int[] historyOfChoicesPlayer; historyOfChoicesPlayer = new int [100]; //history of what choises user made
	...
	// Game related variabels
		...
		String cont = "y"; // To allow Player to choose to y to continue game
...
// DECLARE AND INITIALISE OBJECTS
		Scanner sc = new Scanner(System.in);		// used for testing the game before adding in the JOPtion panes
...
//while loop while player chooses Odd or Even
	while (playerChoiceOdds > 1) {
		playerChoiceOdds = Integer.parseInt(JOptionPane.showInputDialog(null, "Enter 1 (Odd) or 0 (Even)\nChoice can not be changed after this point without restarting the game:"));
	}  // player selected a valid option (odd or even)

	// Set text for output messages later
...
//rounds of the game	
	...
	while (cont.equals("y")) {  //while loop until Player chooses not to press Y at end of Game
		
		while (gamePointsTotal < 6)		{  // while loop while Player or Computer have under 6 points.

			// PLAYER INPUT
			//while loop until player chooses 0 to 10
			while (choiceNumberPlayer > 10)  {
				choiceNumberPlayer = Integer.parseInt(JOptionPane.showInputDialog(null,"Choose number to play (0 to 10):"));
			} // end of player inpuy
			...
			historyOfChoicesPlayer[r] = choiceNumberPlayer;	//store computer and player choises - storing user choise
			historyOfChoicesComputer[r] = choiceNumberComputer; 	//store computer and player choises storing - computer choise
			...
			if (choiceNumberPlayer > choiceNumberComputer) {
				...
				playerPointsBonus = 1; 
				} 	// store 1 bonus point to counter.
	...
	cont = JOptionPane.showInputDialog(null,"Do you want to continue (Press Y to continue. Press Any Other key to exit):");
	cont=cont.toLowerCase();   // to allow input in either upper or lower case
	...
	while (r < len) {
		r = r + 1; //adding 1 to round counter.
 		JOptionPane.showMessageDialog(null,"Round: " + r + " - " + historyOddorEven[r] + " - \nYour choice: " + historyOfChoicesPlayer[r]+"\n - Computer choice: " + historyOfChoicesComputer[r]);
  		}
